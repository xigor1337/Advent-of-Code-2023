#there is some mistakes but i can`t find out what and where
...
def check(lista):
    ...
    liczby = [liczba1, liczba2, liczba3, liczba3d, liczba4, liczba4d]
    liczby = [item for item in liczby if item]
    if len(liczby) < 2:
        return "0"
    print(liczby)
    ratio = int(liczby[0]) * int(liczby[1])
    return ratio
    ...
    while all(lines):
        ...
        for znak in znaki:
            ...
            while True:
                ...
                liczba = check(visualization(line, index))
                suma += int(liczba)
                print(suma)
        lines.pop(0)
